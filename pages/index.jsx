...
import Cads from "./components/Cards/index.jsx";
...
export default function Home() {
  ...
  const [filtertype, setFiltertype] = useState("концерт");

  
  function sortingNameTitle(name) {
    setSortingName(name.name)
    setFiltertype(name)
  }
  ...
  return (
    <>
      ...
      <Head>
        <title>Kassir {sortingName}</title>
        <meta name="description" content="Generated by create next app" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/kassir_logo.png" />
      </Head>
      <div className={styles.header}>
        <div className={styles.menu}>
          {modalSearch ? (<div className={styles.modalSearch}>
            <input type="text" onChange={(e) => setFilter(e.target.value)} />
            <div onClick={() => setModalSearch(!modalSearch)} className={styles.FiX}>
              <FiX className={styles.FiX2} />
            </div>
          </div>) : null}
          <div className={styles.menu_block}>
            <div className={styles.menu_block_block1}>
              <div>
                <Image alt="/" src="/kassir_logo.png" width={120} height={1} />
              </div>
            </div>
            <div className={styles.menu_input}>
              <button
                onClick={() => setModalSearch(!modalSearch)}
                className={styles.BiSearch}
              >
                <BiSearch />
              </button>
              <input onChange={(e) => setFilter(e.target.value)} type="text" placeholder="Поиск.." />
            </div>
            <div className={styles.menu_info1} >
              <div className={styles.menu_info}>
                <a className={styles.menu_info_tel} href="tel:+996 (704) 144 155">
                  <svg
                    width="25"
                    height="25"
                    viewBox="0 0 36 36"
                  ...
                  <span>+996 (704) 144 155</span>
                </a>
                <div className={styles.menu_info_ru}>
                  <Button className={styles.menu_info_button} onClick={onOpen}> Бишкек<IoIosArrowDown className={styles.IoIosArrowDown} /> </Button>
                  <Modal
                  ...
                  >
                    <ModalOverlay />
                    <ModalContent>
                      <ModalHeader>Города в Кыргызстане</ModalHeader>
                      <ModalCloseButton />
                      <ModalBody>
                ...
                <SlBasket className={styles.SlBasket} />
        ...
        <div className={styles.sorting}>
          <Tabs >
            <div className={styles.sorting_Tabs}>
              {data.map((res) => (
                <Tab onClick={() => sortingNameTitle(res)} key={res.name} className={styles.sorting_Tab}>{res.name}</Tab>
              ))}
            </div>
          </Tabs>
        </div>
